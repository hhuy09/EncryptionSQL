...
BEGIN
	...
	BEGIN
		...
		SET @PUBKEY = (SELECT PUBKEY FROM DBO.NHANVIEN WHERE TENDN = @TENDN);
		SELECT MANV, HOTEN, EMAIL, CONVERT(VARCHAR(MAX), DECRYPTBYASYMKEY(ASYMKEY_ID(@PUBKEY),LUONG, CONVERT(NVARCHAR(100), @MK))) "LUONGCB", TENDN, MATKHAU, PUBKEY
		FROM NHANVIEN;
	END	
END
GO
